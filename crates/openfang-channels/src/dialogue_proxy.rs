...
pub struct DialogueProxyAdapter {
    ...
    callback_url: String,
    /// Authentication token (OpenBao Vault token or legacy API key), zeroized on drop.
...
impl DialogueProxyAdapter {
    ...
    /// * `auth_token` - OpenBao Vault token for authentication.
    pub fn new(
        channel_name: String,
        callback_url: String,
        auth_token: String,
    ...
    ///
    /// Note: `user_id` is NOT included in the payload — the channel-router
    /// extracts it from the Vault token's entity alias.
    fn build_payload(
        &self,
        user: &ChannelUser,
